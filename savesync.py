...
def is_synchronized(game):
    ...
    for file in game["files"]:
        try:
            original_filepath = config.expand_path(file["locations"][platform.system()])
            filepath_in_cloud = config.expand_path(os.path.join(path_in_cloud, file["name"]))
            if not (os.path.exists(original_filepath) and os.path.exists(filepath_in_cloud) and os.path.islink(
                    original_filepath) and os.path.abspath(os.readlink(original_filepath)) == os.path.abspath(
                    filepath_in_cloud)):
                return False
        except KeyError:
            pass
    ...
    if os.path.exists(path_in_cloud):
        for file in game["files"]:
            try:
                ...
                if os.path.exists(original_filepath) and os.path.exists(filepath_in_cloud) and (
                            not os.path.islink(original_filepath) or os.readlink(
                                original_filepath) != filepath_in_cloud):
                    return True
            except KeyError:
                pass
    return False
