...
def draw_main_window():
    ...
    for game in games:
        if savesync.is_synchronized(game):
            ...
            state = 'normal'
        if not "selected" in game:
            game["selected"] = IntVar()
            game["selected"].set(1)
        try:
            image = ImageTk.PhotoImage(
                Image.open(os.path.join(config.game_images_folder, game["id"] + ".png")).resize((32, 32),
                                                                                                Image.ANTIALIAS))
        except FileNotFoundError:
            image = None
        checkbutton = ttk.Checkbutton(mainframe, variable=game["selected"], image=image, text=game["name"],
                                      compound=LEFT, takefocus=False, state=state)
...
def synchronize():
    ...
    for game in syncgames:
        if savesync.has_conflicts(game):
            if messagebox.askyesno("Savegame conflict", "Game '" + game[
                "name"] + "' has local saves as well as synchronized saves. Do you want to overwrite the local saves with the synchronized saves?\n\n(No cancels synchronization for this game)"):
                savesync.remove_local_savegame(game)
            else:
                continue
        savesync.move_save_to_cloud(game)
    messagebox.showinfo("Synchronization done",
                        "The selected games are now set up for synchronization and will store their saves in the specified folder.")
    draw_main_window()
...
ttk.Label(folderpicker, text="Please select the folder to which you want your saves synchronized").grid(row=1, column=1,
                                                                                                        columnspan=2,
                                                                                                        sticky=(W, E))
